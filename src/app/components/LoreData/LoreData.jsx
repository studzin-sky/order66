...
export const LoreData = (props) => {
 ...
 /* switch function in useEffect to fetch data and transform it to
 an object displayed to the user */
  ...
  useEffect(() => {
    const fetchLoreHandler = (tileObject) => {
      ...
      switch (tileObject.type) {
        case "people":
          return fetch(`${tileObject.id}?format=json`)// id is an url
            .then((res) => res.json())
            .then((data) => {
  ...
  }, [props.item]); //UseEffect triggers when props.item is changed
